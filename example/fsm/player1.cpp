/*
    ...
    the following command:
        ...
        g++ -I..\..\boost_1_32_0  -o%TEMP%\metaprogram-chapter11-example16.exe
   example16.cpp
...
template <class Transition, class Next> struct event_dispatcher
{
...
template <class Table, class Event> struct generate_dispatcher;
...
template <class Derived> class state_machine
{
  ...
  protected:
    ...
    struct row
    {
        ...
        static void execute(Derived &fsm, Event const &e) { (fsm.*action)(e); }
    };

    friend class default_event_dispatcher;

    template <class Event> int call_no_transition(int state, Event const &e)
    {
  ...
  public:
    template <class Event> int process_event(Event const &evt)
    {
        ...
        typedef typename generate_dispatcher<typename Derived::transition_table,
                                             Event>::type dispatcher;

        // dispatch the event.
        this->state =
            dispatcher::dispatch(*static_cast<Derived *>(this) // CRTP downcast
                                 ,
                                 this->state, evt);
...
struct generate_dispatcher
    : mpl::fold<mpl::filter_view< // select rows triggered by Event
                    Table, boost::is_same<Event, transition_event<_1>>>,
                default_event_dispatcher, event_dispatcher<_2, _1>>
{
};
...
class player : public state_machine<player>
{
  ...
  private:
    ...
    typedef
        ...
        event_dispatcher<row<Stopped, play, Playing, &player::start_playback>,
                         event_dispatcher<row<Paused, play, Playing,
