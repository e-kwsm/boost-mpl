...
template <typename Transition> struct STT_void_row_impl
{
...
template <typename PrevRowImpl, typename Transition>
struct STT_event_row_impl : PrevRowImpl
{
    typedef typename Transition::from_state_t state_t;
    typedef typename Transition::fsm_t fsm_t;
    typedef typename Transition::base_event_t base_event_t;

    static long do_process_event(fsm_t &fsm, long state,
                                 base_event_t const &evt)
    {
...
template <typename PrevRowImpl, typename StateType>
struct STT_state_row_impl : PrevRowImpl
{
...
template <typename PrevRowImpl, typename Transition> struct STT_row_impl
{
    typedef typename mpl::if_<
        boost::is_same<typename PrevRowImpl::state_t,
...
template <typename Transitions> struct STT_impl_gen
{
  private:
    typedef typename mpl::front<Transitions>::type first_;
    typedef typename mpl::fold<Transitions, STT_void_row_impl<first_>,
                               STT_row_impl<_, _>>::type STT_impl_;

  public:
    typedef STT_state_row_impl<STT_impl_, typename STT_impl_::state_t> type;
};
