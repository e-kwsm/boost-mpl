...
template <class Transition> struct transition_event
{
...
// end states.
template <class Transition> struct transition_max_state
{
    typedef
        typename mpl::int_<(Transition::current_state > Transition::next_state)
...
struct dispatch_table
{
  private:
    ...
    BOOST_STATIC_CONSTANT(
        int,
        max_state =
            (mpl::fold<Stt, mpl::int_<initial_state>,
                       mpl::if_<mpl::greater<transition_max_state<_2>, _1>,
                                transition_max_state<_2>, _1>>::type::value));
    ...
    struct init_cell
    {
        ...
        template <class Transition> void operator()(Transition const &) const
        {
    ...
    dispatch_table()
    {
            ...
            for (int i = 0; i <= max_state; ++i) {
                ...
                cell call_no_transition =
                    &state_machine<Fsm>::call_no_transition;
                entries[i] = call_no_transition;
            }

        // Go back and fill in cells for matching transitions.
        mpl::for_each<
            mpl::filter_view<Stt, boost::is_same<transition_event<_>, Event>>>(
            init_cell(this));
    }
...
template <class Derived> class state_machine
{
  public: // Member functions
    ...
    // transitions.
    template <class Event> int process_event(Event const &evt)
    {
        ...
        // Call the action
        return this->m_state = table::instance.entries[this->m_state](
                   *static_cast<Derived *>(this), this->m_state, evt);
    }

    // Getter that returns the current state of the FSM
    int current_state() const { return this->m_state; }
  ...
  private:
    ...
    // FSM class.
    template <class Event> int no_transition(int state, Event const &e)
    {
        BOOST_ASSERT(false);
        return state;
    }
  ...
  protected: // interface for the derived class
    ...
    state_machine()
        : m_state(Derived::initial_state) // Construct with the default
                                          // initial_state
    {
    }

    // Template used to form rows in the transition table
    template <int CurrentState, class Event, int NextState,
...
struct cd_detected
{
    cd_detected(std::string name, std::vector<std::clock_t> durations)
        : name(name), track_durations(durations)
    {
    }
...
// Testing utilities.
//
static char const *const state_names[] = {"Empty", "Open", "Stopped", "Playing",
                                          "Paused"};
...
void test()
{
    ...
    p.process_event(cd_detected(
        "louie, louie", std::vector<std::clock_t>(/* track lengths */)));
    pstate(p);
