...
class player : public fsm::state_machine<player>
{
  public:
    player() {}
    ...
    // MWCW 8.1 is too eager in inforcing access for non-type template
    // parameters private:
    ...
    bool do_play(play_event const &)
    {
        std::cout << "player::do_play\n";
        return true;
    }
    bool do_stop(stop_event const &)
    {
        std::cout << "player::do_stop\n";
        return true;
    }
    bool do_pause(pause_event const &)
    {
        std::cout << "player::do_pause\n";
        return true;
    }
    bool do_resume(play_event const &)
    {
        std::cout << "player::do_resume\n";
        return true;
    }
    ...
    typedef mpl::list<
        ...
        transition<paused, stop_event, stopped, &player::do_stop>>::type
        transition_table;
