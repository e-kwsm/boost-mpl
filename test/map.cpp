...
MPL_TEST_CASE()
{
    typedef map2<mpl::pair<int, unsigned>, mpl::pair<char, unsigned char>> m_;
...
template <class M> void test()
{
    ...
    BOOST_MPL_ASSERT_NOT(
        (is_same<BOOST_DEDUCED_TYPENAME find<M, mpl::pair<int, int *>>::type,
                 not_found>));
    BOOST_MPL_ASSERT_NOT(
        (is_same<BOOST_DEDUCED_TYPENAME find<M, mpl::pair<long, long *>>::type,
                 not_found>));
    BOOST_MPL_ASSERT_NOT(
        (is_same<BOOST_DEDUCED_TYPENAME find<M, mpl::pair<char, char *>>::type,
                 not_found>));
    BOOST_MPL_ASSERT(
        (is_same<BOOST_DEDUCED_TYPENAME find<M, int>::type, not_found>));
};
...
MPL_TEST_CASE()
{
    ...
    BOOST_MPL_ASSERT(
        (is_same<deref<iter_to_1_pair>::type, mpl::pair<int, int *>>));

    typedef map<mpl::pair<int, int *>, mpl::pair<long, long *>,
                mpl::pair<char, char *>>
        mymap;
...
MPL_TEST_CASE()
{
    typedef mpl::erase_key<
        mpl::map<mpl::pair<char, double>, mpl::pair<int, float>>, char>::type
        int_to_float_map;

    typedef mpl::insert<int_to_float_map, mpl::pair<char, long>>::type
        with_char_too;

    BOOST_MPL_ASSERT(
        (boost::is_same<mpl::at<with_char_too, char>::type, long>));
}
...
MPL_TEST_CASE()
{
    typedef erase_key<map<pair<float, float>, pair<int, int>>, float>::type
        little_map;
