...
template <typename s> void empty_set_test()
{
...
template <typename s> void int_set_test()
{
...
template <typename s> void int_char_set_test()
{
...
template <typename s> void int_char_long_set_test()
{
...
void basic_set_test()
{
    ...
    empty_set_test<BOOST_DEDUCED_TYPENAME erase_key<
        BOOST_DEDUCED_TYPENAME erase_key<S2, char>::type, int>::type>();

    empty_set_test<BOOST_DEDUCED_TYPENAME erase_key<
        BOOST_DEDUCED_TYPENAME erase_key<
            BOOST_DEDUCED_TYPENAME erase_key<S3, char>::type, long>::type,
        int>::type>();
    ...
    int_char_set_test<BOOST_DEDUCED_TYPENAME insert<
        BOOST_DEDUCED_TYPENAME insert<S0, char>::type, int>::type>();
    ...
    int_char_long_set_test<BOOST_DEDUCED_TYPENAME insert<
        BOOST_DEDUCED_TYPENAME
            insert<BOOST_DEDUCED_TYPENAME insert<S0, char>::type, long>::type,
        int>::type>();

    int_char_long_set_test<BOOST_DEDUCED_TYPENAME insert<
        BOOST_DEDUCED_TYPENAME insert<S1, long>::type, char>::type>();

    int_char_long_set_test<BOOST_DEDUCED_TYPENAME insert<S2, long>::type>();
}

template <typename S1, typename S2> void numbered_vs_variadic_set_test()
{
...
template <typename s> void empty_set_types_variety_test()
{
    MPL_ASSERT_NOT((has_key<s, char>));
    MPL_ASSERT_NOT((has_key<s, int>));
...
template <typename s> void set_types_variety_test()
{
...
MPL_TEST_CASE()
{
    ...
    typedef set<char, int const, long *, UDT *const, incomplete, abstract,
...
template <class S> void find_test()
{
    MPL_ASSERT_RELATION(size<S>::value, ==, 3);

    typedef typename end<S>::type not_found;
    BOOST_MPL_ASSERT_NOT(
        (is_same<BOOST_DEDUCED_TYPENAME find<S, int>::type, not_found>));
    BOOST_MPL_ASSERT_NOT(
