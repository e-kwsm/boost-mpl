...
struct upper_bound : find_if<Sequence, bind2<pred_, T, _>>
{
};
...
struct upper_bound_step_impl
{
    ...
    typedef typename apply2<Predicate, T, typename deref<middle_>::type>::type
        cond_;

    typedef typename prior<minus<Distance, offset_>>::type step_;
    typedef upper_bound_step<offset_, Predicate, T, DeferredIterator>
        step_forward_;
...
struct upper_bound
{
  ...
  public:
    typedef
        typename aux::upper_bound_step<size_, pred_, T, begin<Sequence>>::type
            type;
};
