...
// constants) and GCC/PGI/Clang (which issues "unused variable" warnings when
// static constants are used at a function scope)
#if BOOST_WORKAROUND(BOOST_BORLANDC, BOOST_TESTED_AT(0x610)) ||                \
...
template <bool C> struct assertion
{
...
template <class Pred> struct extract_assert_pred;

template <class Pred> struct extract_assert_pred<void(Pred)>
{
    typedef Pred type;
};

template <class Pred> struct eval_assert
{
...
template <class Pred> struct eval_assert_not
{
    typedef typename extract_assert_pred<Pred>::type P;
    typedef typename P::type p_type;
    typedef typename ::boost::mpl::if_c<
...
template <typename T> T make_assert_arg();
