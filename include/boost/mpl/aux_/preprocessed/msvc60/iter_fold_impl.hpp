...
template <int N> struct iter_fold_chunk;
...
template <int N> struct iter_fold_chunk
{
    ...
    struct result_
    {
        ...
        typedef iter_fold_impl<((N - 4) < 0 ? 0 : N - 4),
...
template <typename Last, typename State> struct iter_fold_null_step
{
    ...
    struct result_
    {
        typedef
            typename if_<typename is_same<First, Last>::type,
                         ...
                         iter_fold_step<First, Last, State, ForwardOp>>::type
                res_;
...
struct iter_fold_step
{
    typedef iter_fold_chunk<-1>::template result_<
        ...
        typename apply2<ForwardOp, State, First>::type, ForwardOp>
        chunk_;
...
template <int N, typename First, typename Last, typename State,
          typename ForwardOp>
struct iter_fold_impl
    : iter_fold_chunk<N>::template result_<First, Last, State, ForwardOp>
{
};
