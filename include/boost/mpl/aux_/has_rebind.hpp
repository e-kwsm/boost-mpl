...
#if BOOST_WORKAROUND(__EDG_VERSION__, <= 244) &&                               \
...
template <typename T>
struct has_rebind
    : if_<msvc_is_class<T>, has_rebind_impl<T>, bool_<false>>::type
{
};
...
template <typename T> struct has_rebind
{
    ...
    BOOST_STATIC_CONSTANT(bool, value = sizeof(has_rebind_tag<int>() | get()) ==
                                        sizeof(yes_tag));
};
#else  // BOOST_BORLANDC
template <typename T> struct has_rebind_impl
{
    static T *get();
    BOOST_STATIC_CONSTANT(bool, value = sizeof(has_rebind_tag<int>() | get()) ==
                                        sizeof(yes_tag));
...
template <typename T>
struct has_rebind : if_<is_class<T>, has_rebind_impl<T>, bool_<false>>::type
{
};
