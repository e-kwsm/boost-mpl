...
template <typename T, BOOST_PP_ENUM_PARAMS(i, T C)>
...
#define MPL_AUX_LIST_C_TAIL(list, i, C)                                        \
    BOOST_PP_CAT(BOOST_PP_CAT(list, BOOST_PP_DEC(i)),                          \
                 _c)<T, BOOST_PP_ENUM_SHIFTED_PARAMS(i, C)> /**/

template <typename T, BOOST_PP_ENUM_PARAMS(i, T C)>
struct BOOST_PP_CAT(BOOST_PP_CAT(list, i), _c)
    : l_item<long_<i>, integral_c<T, C0>, MPL_AUX_LIST_C_TAIL(list, i, C)>
{
