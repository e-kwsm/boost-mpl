...
struct s_iter_get
    : eval_if<has_key<Set, typename Tail::item_type_>,
              identity<s_iter<Set, Tail>>, next<s_iter<Set, Tail>>>
{
};
...
template <typename Set, typename Tail> struct s_iter : s_iter_impl<Set, Tail>
{
};
...
template <typename Set> struct s_end_iter
{
    typedef forward_iterator_tag category;
...
template <typename Set, typename Tail>
struct s_iter
    : if_<is_same<Tail, set0<>>, s_end_iter<Set>, s_iter_impl<Set, Tail>>::type
{
};
